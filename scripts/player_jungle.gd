...
func coyote_jump():	
	if is_player_on_floor() == false and (motion.y > 0 and motion.y < GRAVITY*10):
		if jumping == false:
			jump()
...
func draw_rope():
	if Input.is_action_pressed("ui_M1"):
		draw_line(get_parent().global_position, get_viewport().get_mouse_position(), Color(0,0,0,1))
